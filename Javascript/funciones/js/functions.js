...
function dniLetter( dni ) {
	...
	var length=dniString.length;
	var isString=isNaN(dni-1)
	if (length===8 &&  isString===false || dni>0) {
...
function findLongestWord( array_words ){
	var lon=array_words.length;
	var arrayLon=[];

	for (i=0; i<lon; i++) {
		arrayLon[i]=array_words[i].length;
	}

	var maximum=Math.max.apply(null, arrayLon);
	return maximum
}
...
array_lengths.push( findLongestWord( ["Mario", "Lucas", "Castells"] ) );
array_lengths.push( findLongestWord( ["Hello", "Hola", "alo", "Bonjour"] ) );
